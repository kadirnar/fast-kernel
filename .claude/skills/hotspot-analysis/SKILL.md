...
1. **GPU busy < 60 % of wall** → the workload is launch/Python bound. The first hypotheses are structural:
   CUDA graphs (whole call or per stage), fusion of elementwise chains, torch.compile reduce-overhead.
   Kernel count per call is the metric to drive down; expect wall ≈ launches × launch latency.
