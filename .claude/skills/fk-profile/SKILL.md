---
name: fk-profile
description: Profile the current candidate of a fast-kernel campaign, rank hotspots by measured headroom, and explain where time goes (launch bound vs compute vs memory). Use for "where is the time going", "profile the model", "what should we optimize next".
...
- the top 5 targets: class, category, boundness, share, roofline efficiency (SOL), headroom, example shapes;
