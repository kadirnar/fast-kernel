---
name: fk-loop
description: Explains and configures the three ways to keep fast-kernel iterating forever - `/loop /fk-experiment` (scheduled wake-ups), the Stop-hook loop (`fast-kernel loop start`), and the headless driver (`fast-kernel auto`). Use for "keep going overnight", "run this in a loop", "never stop".
...
- **Stop-hook loop** — `fast-kernel loop start` creates `.fast-kernel/loop.active`; the project's Stop hook
  ...
  (it lets the session end after 2 consecutive stops without a new experiment). `fast-kernel loop stop`
  ends it. This is the most robust interactive mode.
- **Headless** — `uv run fast-kernel auto [--agents N] [--model ...]` runs `claude -p` once per experiment
  forever (survives session limits, logs agent activity to the dashboard). `fast-kernel pause|resume|stop`.
