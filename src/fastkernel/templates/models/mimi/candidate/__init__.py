...
apply(model, ctx) -> model
    ...
    `model.encode` / `model.decode` (e.g. with CUDA graphs), pre-pack weights, register the hand-written
    CUDA C++ kernels from `candidate/kernels/`. The returned object must keep the public API:
...
Notes
-----
- Kernels are hand-written CUDA C++ (`fastkernel.backends.cuda_cpp.load_cuda_inline`) captured with CUDA
  graphs; see AGENTS.md for why the tile DSLs are not offered.
