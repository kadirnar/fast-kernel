...
BACKENDS = ["torch", "cuda-graphs", "torch.compile", "triton", "tilelang", "cute-dsl", "cuda-cpp", "hub-kernels"]
...
TECHNIQUES: list[Technique] = [
    ...
    Technique("cuda-graphs", "Capture the workload (or each stage) into CUDA graphs", 0,
              ("launch-bound", "sequential"), ("latency", "any"), ("cuda-graphs",), 3.0, "low", "cuda-graphs",
    ...
    Technique("torch-compile", "torch.compile (inductor) with mode=max-autotune / reduce-overhead", 0,
              ("launch-bound", "elementwise", "norm"), ("latency", "memory", "any"), ("torch.compile",), 1.8, "low",
              "torch-compile",
              "Inductor fuses elementwise/norm chains and, with reduce-overhead, wraps the graph in CUDA graphs. Cheap to try; "
              "compile time is not counted in latency but recompiles on dynamic shapes are.",
              ("Compile the top-level callable or the hot submodules only (dynamic=False).",
               "Check for graph breaks with TORCH_LOGS=graph_breaks; remove data-dependent Python in the hot path.")),
    Technique("kernel-count-reduction", "Reduce kernel count: fuse elementwise chains, remove copies/casts", 0,
              ("launch-bound", "elementwise", "memory-movement"), ("latency", "memory"), ("triton", "torch.compile", "cuda-cpp"),
              2.0, "low", "triton-kernels",
    ...
    Technique("weight-prepack", "Pre-cast / pre-transpose / pre-fold weights at load time", 0,
              ("gemm", "conv", "norm", "memory-movement"), ("memory", "latency"), ("torch",), 1.3, "low", "torch-compile",
    ...
    Technique("dtype-policy", "Lower-precision compute with fp32 accumulation where the oracle tolerates it", 0,
              ("gemm", "conv", "attention"), ("compute", "memory"), ("torch", "triton", "tilelang", "cute-dsl"), 1.7, "medium",
    ...
    Technique("block-tuning", "Block-size / num_warps / num_stages sweep (autotune)", 1,
              ("gemm", "conv", "attention", "norm", "reduction"), ("compute", "memory"), ("triton", "tilelang", "cute-dsl"),
              1.3, "low", "triton-kernels",
    ...
    Technique("memory-access", "Coalescing, vectorized loads, channels-last, L2 swizzle, prefetch", 2,
              ("gemm", "conv", "norm", "elementwise", "reduction", "indexing"), ("memory",), ("triton", "tilelang", "cute-dsl", "cuda-cpp"),
              1.3, "low", "triton-kernels",
    ...
    Technique("epilogue-fusion", "Fuse bias/activation/residual/cast/quant into the GEMM or conv epilogue", 3,
              ("gemm", "conv", "elementwise"), ("memory", "latency"), ("triton", "tilelang", "cute-dsl", "cuda-cpp"), 1.5, "low",
              "triton-kernels", "Avoid writing and re-reading the intermediate; typical for MLP (act*gate), conv+ELU, residual adds.",
              ()),
    Technique("implicit-gemm-conv", "Implicit-GEMM convolution on tensor cores", 3,
              ("conv",), ("compute", "memory"), ("triton", "tilelang", "cute-dsl"), 2.0, "medium", "triton-kernels",
    ...
    Technique("fused-attention", "Fused attention (flash-style online softmax) or the right SDPA backend", 3,
              ("attention",), ("compute", "memory", "latency"), ("torch", "triton", "tilelang", "hub-kernels"), 1.6, "medium",
              "triton-kernels", "For short sequences a single fused kernel (QKV proj + attention + out proj) beats 6 launches; for "
    ...
    Technique("fused-norm", "Fused RMSNorm/LayerNorm (+ optional residual and quant) kernels", 3,
              ("norm",), ("memory", "latency"), ("triton", "hub-kernels", "torch.compile"), 2.5, "low", "triton-kernels",
    ...
    Technique("fused-quantizer", "Fused codebook distance + argmin (VQ / RVQ search)", 3,
              ("quantizer", "reduction", "sequential"), ("memory", "latency"), ("triton", "cuda-cpp", "tilelang"), 3.0, "medium",
              "triton-kernels",
    ...
    Technique("fused-elementwise", "Fuse activation / gating / residual chains into one kernel", 3,
              ("elementwise", "memory-movement"), ("memory", "latency"), ("triton", "torch.compile"), 2.0, "low", "triton-kernels",
    ...
    Technique("split-k", "Split-K / stream-K for skinny GEMMs and small batch", 4,
              ("gemm", "conv"), ("compute", "latency"), ("triton", "tilelang", "cute-dsl", "cuda-cpp"), 1.4, "medium",
              "triton-kernels", "When M*N tiles cannot fill the SMs, split the reduction across CTAs and reduce in fp32.", ()),
    Technique("persistent-kernel", "Persistent / megakernel with grid barriers for dependent stages", 4,
              ("sequential", "launch-bound", "quantizer"), ("latency",), ("triton", "cuda-cpp"), 1.5, "high", "cuda-cpp-kernels",
    ...
    Technique("warp-specialization", "Warp specialization, TMA, async copies", 4,
              ("gemm", "attention", "conv"), ("compute", "memory"), ("tilelang", "cute-dsl", "cuda-cpp"), 1.3, "high",
              "cute-dsl-kernels", "Producer/consumer warps, TMA loads, cp.async pipelines; large tiles and deep pipelines.", ()),
    # ---- Tier 5: architecture-specific -----------------------------------------------------
    Technique("arch-tuning", "Architecture-specific tuning (SM count, smem size, tensor-core shapes)", 5,
              ("gemm", "conv", "attention"), ("compute",), ("triton", "tilelang", "cute-dsl", "cuda-cpp"), 1.15, "medium",
    ...
    Technique("indexing-fusion", "Fuse embedding/gather with the consumer; avoid index_select copies", 6,
              ("indexing", "memory-movement"), ("memory", "latency"), ("triton", "torch.compile"), 1.5, "low", "triton-kernels", "", ()),
    Technique("decode-step-fusion", "Fuse the autoregressive decode step (norm+proj+attn+mlp) per layer", 6,
              ("sequential", "launch-bound", "gemm"), ("latency", "memory"), ("cuda-graphs", "triton", "torch.compile"), 2.5,
    ...
    Technique("hub-kernel", "Use a pre-built kernel from the Hugging Face kernels hub", 6,
              ("attention", "norm", "elementwise", "gemm"), ("any",), ("hub-kernels",), 1.5, "low", "hub-kernels",
